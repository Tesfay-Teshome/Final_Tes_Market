...
const StorefrontPage: React.FC = () => {
  ...
  return (
    <div
      className="min-h-screen bg-[#070b0f] selection:bg-[var(--store-accent)]/30 pb-20 overflow-x-hidden"
      style={{
        ...themeVars,
    ...
    >
      ...
      <div className="w-full bg-[#0b141a]/95 backdrop-blur-md border-b border-white/5 py-2 px-6 sticky top-0 z-[100] text-white">
      ...
      <div className="relative">
        {/* Banner with Gradient Layering */}
        <div className="absolute inset-0 z-0 h-[750px] overflow-hidden">
          {store?.banner_url ? (
        ...
        <div className="relative z-10 text-white">
          ...
          <div ref={productsSectionRef} className="max-w-7xl mx-auto px-6 lg:px-12 pb-32">
            ...
            <div className="mt-32 grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="rounded-[3rem] overflow-hidden relative aspect-video group">
                <img src="https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?w=800&q=80" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[2s]" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#070b0f] via-transparent to-transparent" />
                <div className="absolute bottom-10 left-10">
                  <span className="text-xs font-black text-[var(--store-accent)] uppercase tracking-widest mb-2 block">Our Workshop</span>
                  <h4 className="text-2xl font-bold text-white">Ethically Handcrafted</h4>
                </div>
              </div>
              <div className="rounded-[3rem] overflow-hidden relative aspect-video group">
                <img src="https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=800&q=80" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[2s]" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#070b0f] via-transparent to-transparent" />
                <div className="absolute bottom-10 left-10">
                  <span className="text-xs font-black text-[var(--store-accent)] uppercase tracking-widest mb-2 block">Our Commitment</span>
                  <h4 className="text-2xl font-bold text-white">Carbon Neutral Delivery</h4>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-col md:flex-row gap-10 items-start">
                  <div className="w-40 h-40 rounded-full overflow-hidden border-4 border-white/5 shrink-0 grayscale hover:grayscale-0 transition-all duration-700">
                    <img src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&h=300&q=80" className="w-full h-full object-cover" />
                  </div>
                  <div className="space-y-6 flex-1">
                    <div className="flex items-center gap-3 text-sm font-bold text-[var(--store-accent)] uppercase tracking-widest mb-4">
                      <MapPin className="w-4 h-4" /> {store?.socials?.makerIdentity || "Handmade with Love"}
                    </div>
                    <p className="text-lg text-white/70 leading-relaxed italic border-l-2 border-[var(--store-primary)]/30 pl-8 bg-white/[0.02] py-4 rounded-r-2xl">
                      "{store?.socials?.valueProposition || store?.about || "We believe in the power of nature to heal and rejuvenate."}"
                    </p>
                    <div className="grid grid-cols-2 gap-8 pt-6 font-black uppercase tracking-widest">
                      <div>
            ...
            <div className="mt-24 pt-16 border-t border-white/5 flex flex-col items-center">
              <div className="flex flex-col items-center mb-16">
                <div className="w-20 h-20 rounded-full border border-white/10 p-1 mb-8 opacity-60 hover:opacity-100 transition-all duration-700">
                  {store?.logo_url ? <img src={store.logo_url} className="w-full h-full object-cover rounded-full" /> : <Store className="w-full h-full text-white/10" />}
                </div>
                <h3
                  className={`${botanicalSerif} text-4xl md:text-5xl font-medium text-white mb-12 uppercase tracking-[0.1em]`}
                  style={{ fontFamily: `var(--store-heading-font), serif` }}
                >
                  {store?.display_name || slug}
                </h3>
                <div className="flex flex-wrap justify-center gap-x-16 gap-y-6 text-xs font-bold uppercase tracking-[0.2em] text-white/30">
                  {['Collection', 'Policies', 'Privacy', 'Contact'].map(m => (
                    <Link key={m} to="#" className="hover:text-[var(--store-accent)] transition-colors relative group">
                      {m}
                      <span className="absolute -bottom-2 left-0 w-0 h-px bg-[var(--store-accent)] transition-all group-hover:w-full" />
                    </Link>
                  ))}
                </div>
              </div>
              <div className="w-full flex flex-col md:flex-row items-center justify-between gap-8 py-10 border-t border-white/5 text-[10px] font-bold text-white/20 uppercase tracking-[0.3em]">
                <div>&copy; {new Date().getFullYear()} {store?.display_name || slug}. All Rights Reserved.</div>
                <div className="flex items-center gap-8">
                  ...
                  <span>Powered by Tes Market</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <AnimatePresence>
        {showContactModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-2xl bg-[#0b141a] rounded-[3rem] p-12 border border-white/10 shadow-2xl relative overflow-hidden">
              <div className="flex items-center justify-between mb-10">
                <h3 className="text-3xl font-black text-white uppercase tracking-tight">Contact Vendor</h3>
                <button onClick={() => setShowContactModal(false)} className="p-4 rounded-full bg-white/5 hover:bg-white/10 transition-colors"><X className="w-6 h-6 text-white" /></button>
              </div>
              <div className="grid gap-4 font-black uppercase tracking-widest text-[11px]">
                <button className="flex items-center gap-6 p-6 bg-white/5 rounded-2xl hover:bg-white/10 transition-all text-white group">
                  <MessageCircle className="w-6 h-6 text-[var(--store-accent)] group-hover:scale-110 transition-transform" /> Chat Now
                </button>
                <a href={`mailto:${store?.socials?.supportEmail || 'support@example.com'}`} className="flex items-center gap-6 p-6 bg-white/5 rounded-2xl hover:bg-white/10 transition-all text-white group">
                  <Mail className="w-6 h-6 text-[var(--store-accent)] group-hover:scale-110 transition-transform" /> Send Email
                </a>
              </div>
            </motion.div>
          </motion.div>
